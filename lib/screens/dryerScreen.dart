...
class DryerScreen extends StatefulWidget {
  @override
  _DryerScreenState createState() => new _DryerScreenState();
}
...
class _DryerScreenState extends State<DryerScreen> {
  ...
  Future<String> getData() async {
    ...
    setState(() {
      for (int i = 0; i < facilities.length; i++) {
        if (resBody[i + 5]["status"] == "available") {
          facilities[i] = "利用可能";
          ...
          nameFacilites[i] = resBody[i + 5]["user"];
        }
      }
    });
    return 'Success!!';
  ...
  Future<String> changeStatus(int id, String status) async {
    ...
    var resBody = JSON.decode(response.body);
    setState(() {
      facilities[id - 6] = resBody["status"];
    });
    await getData();
    return "Successs";
  }
  ...
  Future<Null> _askUser(int id) async {
    String status = (this.facilities[id - 6] == "利用可能") ? "on" : "off";
